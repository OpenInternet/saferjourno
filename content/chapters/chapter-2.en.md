...
- A phishing attack is commonly used by criminals as a way of obtaining personal data on someone, such as account login details, or financial information. A message is sent to a user encouraging them to click on a link, this link contains malware that once downloaded onto a device can obtain sensitive data or lock the user out of their computer. Phishing attacks are commonly used by criminals and target many people at the same time using the same message.
- A common type of phishing attack involves getting a user to click on a link that takes them to a fake login page for a well-known company, such as an email provider. Once the user enters their login details this information is then recorded by the attacker and is used to gain access to their accounts.
- A spear phishing attack is an attack directed at an individual or small group of people. The adversary will normally spend time studying the person or people and collecting personal data on them. This information will then be included in a message sent to the target/s along with a malicious link. Spear phishing attacks can be very difficult to spot because they have been designed with the target in mind. These types of attacks are commonly used by governments and online hacking groups to target journalists.
...
- Encourage them to research the online services they use/want to use and who owns them, how they store their data, where that data is stored, and whether they share data with others. Highlight how this could be a risk for them.
