...
- Drive technology is constantly evolving. Solid-state drives are generally faster but more expensive, while hard drives are cheaper but can be damaged more easily when dropped or handled roughly (for example during travel). There is some anecdotal evidence of solid-state drives which have been used a lot starting to lose data when powered off for prolonged periods of time. Since the technology is changing so often, we recommend that journalists and trainers take a moment to research it prior to choosing which drive for backups and long-term storage.
