...
- The most secure form of communication is to use end-to-end encryption by default. This means that the information is automatically encrypted both in transit and on the server. Some examples include WhatsApp and Signal. Other services (such as Telegram) only enable end-to-end encryption if the user turns it on explicitly, so journalists should ensure that they have activated the encryption.
...
- Some messaging apps, such as Signal WhatsApp, allow for end-to-end encrypted backups to the cloud. The encryption means that, even though the chat histories might be backed up to a cloud such as Signal's cloud, iCloud, or Google Drive, the organizations which manage those cloud will not be able to decrypt those conversations if they do not have the recovery key.
  - Signal is soon [rolling out encrypted cloud backups](https://signal.org/blog/introducing-secure-backups/), though currently those backups can only be done to Signal's own servers. Signal's services include a free tier that saves 45 days of media history and all text message histories, and a paid tier with additional storage for media. Your backups are protected by a 64 character recovery key. Without that key, you will not be able to restore the backup. Unlike WhatsApp, Signal does not offer the option of unencrypted cloud backups.
  - WhatsApp offers both encrypted and unencrypted backups to the cloud. We strongly recommend that journalists who use WhatsApp for any sensitive communications only use [the encrypted cloud backups](https://faq.whatsapp.com/490592613091019). Much like Signal, WhatsApp's encrypted cloud backups are also protected by a password of 64-character key.
